...
- [ ] **Step 1** 失败测试：render SeoLayout（MemoryRouter + index route stub）→ RunningHead 文案与 Footer 导航链接（/files /lab /about）存在。
- [ ] **Step 2** 实现三组件 + routes.ts 重构 + prerender 固定路由；ShowcasePage 切换到布局共享件（局部 RunningHead/Footer 删除，Footer 文案并入 SiteFooter）。
- [ ] **Step 3** 门禁四连 + build 冒烟（8 页 prerender）；提交 `feat(pages): shared seo layout, running head, site footer (P3/T1)`。
...
- [ ] **Step 1** 失败测试：folioPage 七态（0/0.5/1/负值/超1/total=0/取整边界）；romanNumeral（1/4/9/14/40/90/400/2026）；渲染断言（folio 文本格式、aria-label；ChapterNumeral 带 text-outline 类且 aria-hidden）。
- [ ] **Step 2** 实现；门禁四连；提交 `feat(motion): reading folio and chapter numeral primitives (P3/T2)`。
...
- [ ] **Step 1** 扩展测试：folio 文本出现且 total 正确（3 min read → "∕ 03"）；numeral 罗马字匹配该文序号。
- [ ] **Step 2** 实现；门禁四连；提交 `feat(pages): case-file reading template with folio chrome (P3/T3)`。
...
- [ ] **Step 1** 失败测试：groupByWorld 归组与排序；渲染 → 3 张已发布卡、draft 缺席、卡链接 href 正确、world=red 时 RED WORLD 轨标题在前。
- [ ] **Step 2** 实现；门禁四连；提交 `feat(pages): the archive wall grouped by world (P3/T4)`。
...
- [ ] **Step 1** 结构测试：页标题、"EVIDENCE LAB" 文案、关键演示位锚点（aria-label/文案）存在。
- [ ] **Step 2** 实现；门禁四连；提交 `feat(pages): evidence lab motion showroom (P3/T5)`。
...
- [ ] **Step 1** 测试：标题与署名 "Orion Arch"、版本表键值、RSS 链接。
- [ ] **Step 2** 实现；门禁四连；提交 `feat(pages): colophon about page (P3/T6)`。
...
- [ ] **Step 1** 扩展 showcase 测试：3 篇标题与 archive 链接出现。
- [ ] **Step 2** 实现；门禁四连；提交 `feat(pages): cover gets latest-files entry (P3/T7)`。
...
```
- [ ] **Step 2** 视觉抽查：/files 两世界分组、正文页 folio + 数字幕、/lab 演示位、/about 署名。
- [ ] **Step 3** 分支 `magpie-p3-pages` 推送开 PR；合并后勾选状态随 PR 入库。
